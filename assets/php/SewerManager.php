...
class SewerManager
{
    ...
    {
        $query = $this->db->query("SELECT sum(t.amount) as amount FROM sewers s LEFT JOIN tasks t ON s.id = t.sewer_id WHERE s.id = $q");
        if ($query->rowCount() > 0) {
    ...
    {
        $query = $this->db->query("SELECT sum(t.quantity) as quantity FROM sewers s LEFT JOIN tasks t ON s.id = t.sewer_id WHERE s.id = $q");
        return $query->rowCount() > 0 ? $query->fetch(PDO::FETCH_ASSOC)['quantity'] : 0;
    }
    ...
    public function addSewer($item, $card)
    {
        $query = $this->db->prepare("INSERT INTO sewers (card_number, item) VALUES ($card, '$item')");
        if ($query->execute()) {
    ...
    public function updateTask($id, $status, $employee, $date, $qty, $u_p, $amt)
    {
        $query = $this->db->prepare("UPDATE tasks SET employee = :employee, date = :date, quantity = :qty, unit_price = :u_p, amount = :amt WHERE sewer_id = $id AND status = $status");
        $null = null;
        if ($employee == "") {
        ...
        if ($qty == "") {
            $query->bindValue(":qty", $null, PDO::PARAM_NULL);
        } else {
            $query->bindValue(":qty", $qty);
        }
        if ($u_p == "") {
            $query->bindValue(":u_p", $null, PDO::PARAM_NULL);
        } else {
            $query->bindValue(":u_p", $u_p);
        }
        if ($amt == "") {
            $query->bindValue(":amt", $null, PDO::PARAM_NULL);
        } else {
            $query->bindValue(":amt", $amt."f");
        }
        if ($date == "") {
